...
import { debounce, filter, has } from "lodash-es";
import { ReactElement, useContext, useEffect, useRef, useState } from "react";
import { BsDash } from "react-icons/bs";
import { TbCheck, TbFileExport } from "react-icons/tb";
import {
  Link,
  NavLink,
  Outlet,
...
interface Props<T> {
  ...
  onRow?: (data: any) => { rowClassName: string };
...
export default function EntityList<T>(props: Props<T>) {
  ...
  const hasPageBeenRendered = useRef({ filterEffect: false });
  ...
  useEffect(() => {
    if (collection && searchParams?.size === 0) {
      let filters: string[] = [];
      collection?.[0]?.collection?.filter?.forEach((item) => {
        item?.forEach((filter) => {
      ...
      if (filters.length > 0) {
        onFilterChange(filterQuery(filters));
  ...
  // useEffect(() => {
  //   if(hasPageBeenRendered.current.filterEffect||filterValue.length>0){
  //   onFilterChange(filterQuery(filterValue));
  //   }
  //   hasPageBeenRendered.current.filterEffect=true;
  // }, [filterValue]);
          ...
          if (!Array.isArray(col.key)) {
            if (col?.isDate) {
              return (data[`${col.key}`] = item[`${col.key}`]
                ? dayjs(item[`${col.key}`]).format("MMM-DD, YYYY")
                : "");
            } else {
              return (data[`${col.key}`] = item[`${col.key}`]);
            }
            ...
            if (col?.isDate) {
              return (data[`${col.key}`] = childeView(item, col.key)
                ? dayjs(childeView(item, col.key)).format("MMM-DD, YYYY")
                : "");
            } else {
              return (data[`${col.key}`] = childeView(item, col.key));
            }
  ...
  return (
    <div
      className={
        `h-full rounded-lg shadow-md border-t border-gray-50 flex space-x-2  relative px-2 ` +
        parentStyle
      }
    >
      <div
      ...
      >
        <div className="h-14 bg-white  flex items-center justify-between px-3 space-x-2">
          <div className={`h-full w-full flex items-center ${fullScreen?"md:text-xl text-lg":"text-xl"} font-bold `}>
            {title}
          </div>
          {fullScreen &&
             (<div className="h-full w-full justify-end md:flex hidden space-x-2 items-center">
            {filterValue.length > 0 &&
              ...
              showResetFilter && (
                ...
                >
                  <span className="shrink-0 text-black cursor-pointer">
                    Reset Filters
                  </span>
                </div>
              )}
            ...
            {showExport !== false && (
              <Dropdown menu={exportMenu} className="hidden">
                <Space>
                  <Button
                    className="shadow-none flex items-center  rounded "
                    icon={<TbFileExport />}
                    type="primary"
                  >
                    <span>Export</span>
          ...
          <div
            ...
            >
              ...
              >
                ...
                >
                 {fullScreen ?(
                 <span className="hidden md:block">{newButtonText ?? "New"}</span>
                 ):(<span>New</span>)}
        ...
        <div
          className={` bg-white p-2 py-6 w-full ${
            viewMode === "list"
              ? showExport !== false
                ? "h-10 flex items-center justify-between"
                : "h-10 flex w-full items-center justify-start"
              : "flex-col space-y-2"
          }`}
        >
          {/* <div
            className={`h-full flex items-center ${showNewButton === false ? "invisible" : "visible"
              }`}
          >
            <Link
              to={`${setting?.rootUrl}/new`}
            ...
            >
              <Button
                className="!bg-black !text-white shadow-none rounded flex  items-center justify-center"
                // color="default"
                // variant="solid"
                icon={<PlusOutlined />}
              >
          ...
          <div
            className={`flex ${viewMode === "list" ? "w-2xl" : ""} space-x-2 `}
          >
            <Input.Search
            ...
            {setting?.filter && filterMenus !== undefined && (
              <>
                <Checkbox.Group
                  ...
                  onChange={(data: any) => {
                    setFilterValue(data);
                    onFilterChange?.(filterQuery(data));
                  }}
                >
                  <Dropdown menu={filterMenus} trigger={["click"]}>
                    ...
                    >
                      ...
                      <span className={`${viewMode === "detail" && "hidden"} `}>
                        Filter
                      </span>
                    </Button>
                  </Dropdown>
        ...
        <div className="px-2 bg-white overflow-x-auto relative">
          <Spin spinning={itemsLoading}>
            <table className="w-full text-sm mt-2 text-left">
              ...
              <tbody
                className={"relative text-sm text-gray-600 border border-white"}
              >
                {items?.map((item, idx) => {
                  const rowClassName = onRow?.(item)?.rowClassName;
                  return (
                    <tr
                      onDoubleClick={() => {
                      ...
                      className={`${rowClassName ?? ""} group ${
                        ...
                        viewMode === "detail"
                          ...
                          : ""
                      } border-b border-b-gray-300 hover:bg-gray-50`}
                    >
                      {check && (
                        <td
                          className={`${
                      ...
                      {viewMode === "list" ? (
                        setting?.visibleColumn?.map(
                          (col, index) =>
                            col.hide !== true && (
                              <td
                                key={index}
                                className={`py-2 px-2 ${col.tdClass ?? ""}`}
                              >
                                {col.render ? (
                                  <>{col.render(item)}</>
                        ...
                        <th
                          className={`py-2 group-hover:bg-primary group-hover:text-white w-full px-2 font-medium  whitespace-nowrap `}
                        >
                          <NavLink
                            style={{ width: "100%" }}
          ...
          {total > 0 && (
            <div className="flex w-full justify-end p-2">
              <Pagination
                ...
                showSizeChanger
                pageSize={props.collectionQuery?.top ?? 10}
      ...
      <div
        ...
        } flex-col space-y-2 p-2 h-full`}
      >
        { !location.pathname.endsWith("/new") &&(
          <div className="mx-4 bg-white p-2 flex justify-between items-center">
          <div className="h-full items-center flex text-xl font-semibold text-black">
            {detailTitle ?? title}
          </div>
        ...
        <div className="h-full bg-white flex space-x-2">
          <Outlet />
        </div>
      </div>
                    ...
                    <td key={index} className="py-2 px-2">
                      {col.render && col?.print !== false ? (
                        ...
                        typeof item[`${col.key}`] === "boolean" ? (
                          ...
                          item[`${col.key}`] ? (
                            dateFormat(item[`${col.key}`], "dd-mmm-yy ")
                          ) : (
                            ""
                          )
                        ) : (
                        ...
                        childeView(item, col.key) ? (
                          dateFormat(childeView(item, col.key), "dd-mmm-yy ")
                        ) : (
                          ""
                        )
                      ) : (
